...
int singleLineCom (char *str, int val) {
    if(str[0] == '/' && str[1] == '/') {
        val = 1;
        return val;
    }
...
int isCommentStart(char *str, int val)
{
    // printf("hell\n");
    if (str[0] == '/' && str[1] == '*')
    {
...
int isCommentEnd(char *str, int val)
{
    // cout << "inside comment end check\n\n";
    if (str[0] == '*' && str[1] == '/')
    {
        // cout << "comment end \n\n";
        val = 0;
...
{
    int left = 0, right = 0;
    int len = strlen(str), val = 0, val_sing = 0;
    ...
    while (right <= len && left <= right)
    {
        ...
        else if (isDelimiter(str[right]) == true && left != right || (right == len && left != right))
        {
            ...
            val_sing = singleLineCom(subStr, val_sing);
            val = isCommentStart(subStr, val);
            val = isCommentEnd(subStr, val);

            // cout << "sub string" << subStr <<endl;

            // // cout << "Comment start: " << isCommentStart(subStr) << endl;
            // // cout << "Comment end: " << isCommentEnd(subStr) << endl;
            // cout << "check comment: " << val << endl;

            // cout << "val_sing" << val_sing << endl;
